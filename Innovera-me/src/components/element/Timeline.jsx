export default function Timeline() {
  ...
  return (
    <div id="timeline" className="bg-black py-12" style={{ backgroundImage: "url('/bg-img.png')", backgroundSize: 'cover' }}>
        ...
        <div className="relative">
          ...
          <div className="space-y-12">
            {timelineEvents.map((event, index) => (
              <div key={event.number} className="relative flex items-start">
                <div className="absolute left-6 md:left-1/2 w-5 h-5 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full transform -translate-x-1/2 shadow-md border-2 border-white" />
                <div
                  className={`relative group ml-12 md:ml-0 md:w-1/2 ${index % 2 === 0 ? "md:pr-8" : "md:pl-8 md:ml-auto"}`}
                >
                  <div className="text-orange-400 font-bold text-xl text-center mb-2">
                    {event.dates}
                  </div>
                  <div className="bg-gray-900 rounded-xl shadow-md p-6 relative">
                    <div className="absolute -left-8 top-1/2 -translate-y-1/2 w-10 h-10 bg-orange-400 flex items-center justify-center text-black font-bold text-lg">
                      {event.number}
                    </div>
                    ...
                    <div className="absolute right-0 top-0 w-2 h-full bg-gradient-to-b from-orange-400 to-orange-600 rounded-r-xl" />
                  </div>
                </div>
              </div>
            ))}
